...
function build_static($post_ID, $post_after, $post_before) {
	...
	exec("cd " . dirname(__FILE__) . " && node dist/postUpdatedHook.js " . escapeshellarg($wpdb->dbname) . " " . escapeshellarg(get_site_url()) . " " . escapeshellarg(get_home_path()) . " " . escapeshellarg($current_user->user_email) . " " . escapeshellarg($current_user->display_name) . " " . escapeshellarg($post_after->post_name) . " > /tmp/wp-static.log 2>&1", $op);
}
